...
public class AspectUtility : MonoBehaviour {
    ...
    private void UpdateCamera() {
		if (!GameManager.FixedAspectRatio || !cam) return;

        float currentAspectRatio = (float)Screen.width / Screen.height;

        // If the current aspect ratio is already approximately equal to the desired aspect ratio,
        // use a full-screen Rect (in case it was set to something else previously)
		if ((int)(currentAspectRatio * 100) / 100.0f == (int)(GameManager.TargetAspectRatio * 100) / 100.0f) {
        ...
        // Pillarbox
		if (currentAspectRatio > GameManager.TargetAspectRatio) {
			float inset = 1.0f - GameManager.TargetAspectRatio / currentAspectRatio;
            cam.rect = new Rect(inset / 2, 0.0f, 1.0f - inset, 1.0f);
        }
        // Letterbox
        else
        {
			float inset = 1.0f - currentAspectRatio / GameManager.TargetAspectRatio;
